...
app = FastAPI(title="PyCalc FX+Crypto API", version="1.1.0")
...
async def fetch_json(url: str, timeout=20):
    """Fetch JSON from URL with timeout & error handling"""
    try:
...
async def get_fiat_rates(base: str):
    base = base.upper()
    key = f"fx:{base}"
    cached = cache.get(key, max_age=60*60)  # cache 1 hour
    if cached:
        return cached
    ...
    if not rates:
        raise HTTPException(status_code=502, detail="No FX rates returned from exchangerate.host")
...
async def get_crypto_simple(coins: List[str], vs: List[str]):
    ...
    cached = cache.get(key, max_age=60*5)  # cache 5 mins
    if cached:
        return cached
    ...
    if not isinstance(data, dict) or not data:
        raise HTTPException(status_code=502, detail="No crypto data from CoinGecko")
